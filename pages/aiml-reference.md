...
Please note that the format attribute is now depreciated but is still included for backwards compatibility. The &lt;from&gt; and &lt;to&gt; tags now automatically parse the date as long as it is in one of the following formats:
...
It is important to note that only English weekday and month names/abbreviations are supported. If you use a language other than English, you must first translate these in order for the tag to parse them correctly.
...
To calculate the difference between the current date and the bot's birthdate, make sure to include a birthdate property in the `.properties` file, in a format that is supported by the &lt;interval&gt; tag. eg: ["birthdate","January 2 1970"]
